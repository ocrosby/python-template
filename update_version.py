"""
This script updates the version in `pyproject.toml.jinja`, `VERSION` file,
`docs/source/conf.py`, `package.json.jinja2`, and `package-lock.json`.
...
def update_version_in_pyproject_toml(version: str) -> None:
    """
    Updates the version in `pyproject.toml.jinja`.
    ...
    update_file(Path("pyproject.toml.jinja"), read_func, write_func)
...
if __name__ == "__main__":
    ...
    files_to_update = {
        Path("pyproject.toml.jinja"): update_version_in_pyproject_toml,
        Path("VERSION"): update_version_in_version_txt,
        Path("docs/source/conf.py"): update_version_in_conf_py,
        Path("package.json.jinja2"): lambda v: update_version_in_json(Path("package.json.jinja2"), v),
